...
// Fixed pointer type
const KEYCHAIN_ID: *const i8 = "7WV56FL599.keychain_tool".as_ptr() as *const i8;
...
pub fn add_keychain_item(service: &str, account: &str, secret: &str) -> Result<(), Error> {
    ...
    let cf_access_group =
        unsafe { CFStringCreateWithCString(ptr::null(), KEYCHAIN_ID, 0x08000100) };

    assert!(
        !cf_service.is_null(),
        "Failed to create CFString for service"
    );
    assert!(
        !cf_account.is_null(),
        "Failed to create CFString for account"
    );
    assert!(!cf_data.is_null(), "Failed to create CFData for secret");
    assert!(
        !cf_access_group.is_null(),
        "Failed to create CFString for access group"
    );
    ...
    unsafe {
        ...
        let status = SecItemAdd(
            attributes_ptr,
            &mut result as *mut *mut c_void as *mut *const c_void,
        );
...
pub fn get_keychain_item(service: &str, account: &str) -> Result<String, Error> {
    ...
    let cf_service = unsafe {
        // CString.as_prt() type is platform dependent
        #[allow(clippy::unnecessary_cast)]
        CFStringCreateWithCString(ptr::null(), service.as_ptr() as *const i8, 0x08000100)
        // UTF-8 encoding
    };
    let cf_account = unsafe {
        // CString.as_prt() type is platform dependent
        #[allow(clippy::unnecessary_cast)]
        CFStringCreateWithCString(ptr::null(), account.as_ptr() as *const i8, 0x08000100)
        // UTF-8 encoding
    };
    ...
    assert!(
        !query_ptr.is_null(),
        "CFDictionary creation failed for query!"
    );
    ...
    unsafe {
        ...
        if status == ERR_SUCCESS {
            assert!(
                !result.is_null(),
                "SecItemCopyMatching returned a null result"
            );
...
extern "C" {
    ...
    fn CFDataCreate(allocator: *const c_void, bytes: *const u8, length: usize) -> *const c_void;
}

fn attributes_to_dict(
    attrs: &[(
        /* key: */ *const c_void,
        /* value: */ *const c_void,
    )],
